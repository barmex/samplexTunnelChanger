...
def get_current_state():
    ...
    if routerInterfaceIndex:
        response = get_current_interface_state(routerHost,routerPort,routerUsername,routerPassword,routerInterfaceIndex)
...
def change_tunnel_interface():
    ...
    if routerInterfaceIndex and ipAddress:
        response = set_tunnel_interface_destination_ip_address(routerHost,routerPort,routerUsername,routerPassword,routerInterfaceIndex, ipAddress=ipAddress)
        return response.text, response.status_code
    else:
        return 'Wrong parameters interfaceIndex or ipAddress.', 500

def get_current_interface_state(routerHost:str, routerPort:int, routerUsername:str, routerPassword:str, routerInterfaceIndex:str) -> dict:
...
def set_tunnel_interface_destination_ip_address(routerHost:str, routerPort:int, routerUsername:str, routerPassword:str, routerInterfaceIndex:str, ipAddress: str) -> str:
    url = f'https://{routerHost}:{routerPort}/restconf/data/Cisco-IOS-XE-native:native/interface/Tunnel={routerInterfaceIndex}/Cisco-IOS-XE-tunnel:tunnel/destination/ipaddress-or-host/'
    headers = {
        'Content-Type': 'application/yang-data+json',
        'Accept': 'application/yang-data+json'
    }
    ...
    if response.status_code == 204:
        print(f'The interface Tunnel{routerInterfaceIndex} has been updated successfully.')
